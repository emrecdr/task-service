...
| `tag`       | string         | —          | Filters to tasks carrying that tag. Repeatable; how repeats combine is governed by `op`, and defaults to narrowing. Matched on `name_key`, so the filter is case-insensitive. Unknown values match nothing (`200`, empty list), consistent with `status`. |
| `op`        | `and \| or`    | `and`      | How **repeated `tag` values** combine. `and` → the task must carry every tag listed; `or` → any one is enough. Applies to `tag` only. It deliberately does **not** apply to `status`: a task holds exactly one status, so an AND across two statuses could never match anything, and repeated `status` therefore always widens. Supplying `op` with no `tag` is accepted and has no effect; any value other than `and`/`or` → 422. |
...
Listeners register at app startup against the `EventBus` the lifespan builds and stores on `app.state`; the outbox relay is what dispatches to it. Adding a Slack notifier in Phase 2 is one file: a new adapter that subscribes to `TaskCompleted`.
