...
The PDF notes the team is *"spread across multiple time zones."* To avoid any ambiguity in stand-ups and audit trails, the service treats time as follows:
...
- `"Fix bug"`, `"fix bug"`, `" FIX BUG "`, `"fix\tbug"` *(only outer whitespace is trimmed; inner whitespace is significant)*.
...
| `limit` | int | `50` | 1–200. Values outside this range → 422. |
| `offset` | int | `0` | ≥ 0. |
...
| `MAX_LIST_LIMIT` | int | `200` | Hard cap on pagination limit. |
...
> *Can this test run with only my feature module imported?* If yes, it's unit and lives in the feature's own `tests/`. If it needs `from app.main import app`, real HTTP, or another feature, it crosses a boundary and lives under `tests/` at the project root.
...
| `schemathesis` *(optional in Phase 1)* | OpenAPI-driven property-based E2E tests. Promoted to a required gate in Phase 2. | CI (optional). |
